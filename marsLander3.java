...
class Player {
    ...
    public static void main(String args[]) {
        ...
        while (true) {
            ...
            // Dans ce cas on procede à deux type de test.
            // Le premier test correspond à la vérification 
            // Le deuxieme test quant à lui vérifie 

            //Ici, on procede à une suite de teste pour satisfaire des conditions
            //en se basant sur une variable de décision 
            // qui défini l'état du système
            //c'est à dire :
                // si variableDeDecision == -1 alors probleme de vitesse horizontale
                // si variableDeDecision == 0 Recherche de la piste d'atterissage
                // si variableDeDecision == 1 Recherche de la piste d'atterissage avec orientation 

            //*** Premier test : Vérification de la vitesse horizontale 
            if (variableDeDecision == -1) {
                if (hSpeed == 0) {
                    variableDeDecision = 1;
                }
                else{
                    variableDeDecision = 0;
                }
            }

            if (variableDeDecision == 0) {
